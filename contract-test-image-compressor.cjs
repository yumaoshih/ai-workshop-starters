...
setTimeout(async () => {
  ...
  // 領域：建立 DataTransfer 並加入 File 物件
  const dt = new win.DataTransfer();
  try {
    dt.items.add(file1);
    dt.items.add(file2);
  } catch (e) {
    // 若 items.add 不支援，改用其他方式
    console.error('DataTransfer.items.add 不支援:', e.message);
  }
  ...
  // 嘗試將 dt.files 賦值給 input.files
  try {
    fileInput.files = dt.files;
  } catch (e) {
    console.error('無法設定 input.files，嘗試其他方式:', e.message);
    // 其他方式：使用 Object.defineProperty 或直接設定
    try {
      ...
      console.error('也無法定義 files 屬性:', e2.message);
    }
  }
